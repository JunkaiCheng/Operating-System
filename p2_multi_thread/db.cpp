...
void Database::copyTable(std::string name,std::string newname) {
    ...
    if (this->tables.find(newname) != this->tables.end())
        throw DuplicatedTableName(
                                  "Error when inserting newtable \"" + newname + "\". Name already exists."
                                  );
    
    //copy the content of table to new table
    
    //this->tables[newname] = std::move(table);
